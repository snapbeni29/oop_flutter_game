...
class Player extends ChangeNotifier {
  ...
  Player(BuildContext context) {
    ...
    _body = new Body(
      ...
      x: 0.0,
      y: (1 - 33 * _pixelHeight) / (1 - 40 * _pixelHeight),
    );
  }
    ...
    Timer.periodic(Duration(milliseconds: 60), (timer) {
      ...
      for (Platform pt in platformList) {
        ...
        // Land onto a platform
            ...
            _body.collideVertically(pt.body, _verticalDirection, speed,
                _pixelWidth, _pixelHeight)) {
          _midJump = false;
          body.y = (getTopBoundary(pt.posY, pt.height, _pixelHeight) -
                  40 * _pixelHeight) /
              (1 - 40 * _pixelHeight);
          collision = true;
          _height = 0.0;
      ...
      if (!collision) {
        ...
        if (_initialHeight - _height > 1.0) {
          _midJump = false;
          body.y = (1 - 33 * _pixelHeight) / (1 - 40 * _pixelHeight);
          _height = 0.0;
          timer.cancel();
        } else
  ...
  Widget displayPlayer() {
    ...
    if (_direction == 'right') {
      return Container(
        width: _body.width * 1.2,
        height: _body.height * 1.2,
        child: img,
      );
    }
